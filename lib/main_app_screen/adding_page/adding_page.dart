...
import '../../back_end/number_formatter.dart';

class AddingPage extends StatefulWidget {
  MomaUser appUser;
  AddingPage({Key? key,required this.appUser}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        appBar: AppBar(
          ...
          title: (const Text(
              "Adding Transaction", style: TextStyle(fontSize: 25,color: Colors.black,fontWeight: FontWeight.bold))),
          actions: <Widget>[
            IconButton(
              icon: const Icon(Icons.check, color: Colors.black,),
              onPressed: () {
                setState(() {
                  if(groupMoney!=UNDEFINED) {
                    widget.appUser.addTransaction(Transaction(
                        int.parse(moneyController.text.replaceAll(RegExp('[^0-9]'), '')),
                        date,
                        groupMoney,
                        noteController.text));
